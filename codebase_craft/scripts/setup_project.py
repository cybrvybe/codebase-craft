...
# Configure logging
logging.basicConfig(filename='project_setup.log', level=logging.INFO, format='%(asctime)s:%(levelname)s:%(message)s')

def install_dependencies(package_json_path: str) -> None:
    os.chdir(package_json_path)
    subprocess.run(['npm', 'install'])

def main(project_name: str, project_directory_structure: str, package_json_template: str, additional_tools_and_libraries: List[str]) -> None:
    ...
    logging.info(f'Created directories based on the YAML template: {project_directory_structure}')
    ...
    logging.info(f'Installed additional tools and libraries: {additional_tools_and_libraries}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('project_directory_structure', type=str, help="The YAML file containing the project directory structure template.")
    parser.add_argument('package_json_template', type=str, help="The JSON file containing the package.json template.")
    parser.add_argument('additional_tools_and_libraries', nargs='*', help="Additional tools and libraries to install.")
    args = parser.parse_args()
    main(args.project_name, args.project_directory_structure, args.package_json_template, args.additional_tools_and_libraries)
